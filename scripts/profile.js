...
_setup.avatar = emoji;
_setup.photoURL = null; // clear custom photo if emoji selected
};
...
uploadWrap.style.cssText = 'grid-column:1/-1;display:flex;flex-direction:column;align-items:center;gap:8px;margin-top:6px';
uploadWrap.innerHTML = `
  <div id="setup-photo-preview" style="width:64px;height:64px;border-radius:50%;background:var(--bg-input);border:2px dashed var(--accent);display:flex;align-items:center;justify-content:center;font-size:24px;cursor:pointer;overflow:hidden;position:relative">
    📷
  </div>
  <span style="font-size:12px;color:var(--text-secondary)">أو ارفع صورة من الجهاز</span>
`;
grid.appendChild(uploadWrap);

const preview = uploadWrap.querySelector('#setup-photo-preview');
preview.onclick = () => {
  ...
  inp.onchange = async (e) => {
    ...
    try {
      const storageRef = storage.ref('avatars/' + _currentUser.uid);
...
if (aviEl) {
  if (userData.photoURL) {
    aviEl.innerHTML = `<img src="${userData.photoURL}" alt="avi"/>`;
  } else {
    aviEl.innerHTML = '';
    aviEl.textContent = userData.avatar || '👤';
  }
}
...
// Photo change button in profile
const changePhotoBtn = document.getElementById('btn-change-photo');
if (changePhotoBtn) {
  changePhotoBtn.onclick = _showChangePhoto;
}
...
await storageRef.put(file);
const url = await storageRef.getDownloadURL();
await db.ref(‘users/’ + _currentUser.uid).update({ photoURL: url });
...
/* ══════════════════════════════
VIEW OTHER USER PROFILE (slide-up)
══════════════════════════════ */
...
```
  ...
  if (aviEl) {
    if (u.photoURL) {
      aviEl.innerHTML = `<img src="${u.photoURL}" alt="avi" style="width:100%;height:100%;object-fit:cover;border-radius:50%"/>`;
    } else {
      aviEl.innerHTML = '';
      aviEl.textContent = u.avatar || '👤';
    }
  }
  ...
  if (badgeRow) {
    ...
    badges.slice(0, 6).forEach(key => {
      ...
      d.style.cssText = 'width:40px;height:40px;display:inline-flex';
