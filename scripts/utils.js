...
/* ── Small badge SVG for chat (28px) ── */
function buildBadgeSVGSmall(key) {
const def = BADGE_DEFS[key];
if (!def) return ‘’;
const full = buildBadgeSVG(key);
// Replace width/height=“56” with “28”
return full.replace(/width=“56” height=“56”/, ‘width=“28” height=“28”’);
}

/* ── Build 3-badge strip for chat messages ── */
function buildMsgBadgesHTML(badges) {
if (!badges || !badges.length) return ‘’;
const top3 = badges.slice(0, 3);
let html = ‘<span class="msg-badges">’;
top3.forEach(key => {
html += ‘<span class="msg-badge-icon" title="' + ((BADGE_DEFS[key] || {}).name || '') + '">’ + buildBadgeSVGSmall(key) + ‘</span>’;
});
html += ‘</span>’;
return html;
}
...
const c = TIER_COLORS[t];
return ` <circle cx="40" cy="40" r="38" fill="${c.outer}"/> ${tierDots(t)} <defs> <radialGradient id="rg_${t}" cx="50%" cy="40%" r="55%"> <stop offset="0%" stop-color="${c.bg1}"/> <stop offset="100%" stop-color="${c.bg2}"/> </radialGradient> </defs> <circle cx="40" cy="40" r="32" fill="url(#rg_${t})"/> <circle cx="40" cy="40" r="32" fill="none" stroke="${c.ring}" stroke-width="2" opacity="0.7"/> ${extraRing}`;
}

function friendsBadgeSVG(tier) {
const extras = tier >= 3 ? `<circle cx="40" cy="40" r="27" fill="none" stroke="${TIER_COLORS[tier].ring}" stroke-width="1" opacity="0.4"/>` : ‘’;
return `<svg width="56" height="56" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg"> ${tierBase(tier, extras)} <circle cx="33" cy="28" r="7" fill="white" opacity="0.95"/> <path d="M18,50 Q18,38 33,38 Q48,38 48,50" fill="white" opacity="0.9"/> <circle cx="50" cy="30" r="5.5" fill="white" opacity="0.75"/> <path d="M37,50 Q38,42 50,42 Q62,42 62,50" fill="white" opacity="0.7"/> ${tier === 4 ? `<polygon points="40,62 46,67 40,74 34,67" fill="#A0DFFF" opacity="0.9"/>` : ''} </svg>`;
}
...
return `<svg width="56" height="56" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg"> ${tierBase(tier)} <rect x="22" y="22" width="36" height="32" rx="5" fill="white" opacity="0.95"/> <rect x="22" y="22" width="36" height="12" rx="5" fill="${calColor}" opacity="0.9"/> <rect x="27" y="18" width="5" height="8" rx="2" fill="${c.dot}"/> <rect x="48" y="18" width="5" height="8" rx="2" fill="${c.dot}"/> <polyline points="30,42 37,50 52,34" fill="none" stroke="${ckColor}" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"/> ${star}${sparkle} </svg>`;
}
...
return `<svg width="56" height="56" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg"> ${tierBase(tier)} <polygon points="40,18 62,34 62,58 18,58 18,34" fill="white" opacity="0.95"/> <polygon points="40,18 62,34 18,34" fill="${roofColor}" opacity="0.9"/> <rect x="33" y="42" width="14" height="16" rx="2" fill="${roofColor}" opacity="0.7"/> <circle cx="57" cy="58" r="10" fill="${badgeColor}" stroke="${tier>=3?c.dot:'white'}" stroke-width="1.5"/> <text x="57" y="62" text-anchor="middle" font-size="9" font-weight="600" fill="${lvColor}" font-family="sans-serif">LV</text> ${star} </svg>`;
}

function adminBadgeSVG() {
const pts = [[40,5],[62,12],[75,34],[75,46],[62,68],[40,75],[18,68],[5,46],[5,34],[18,12]];
return `<svg width="56" height="56" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg"> <circle cx="40" cy="40" r="38" fill="#300010"/> ${pts.map(([x,y]) => `<circle cx="${x}" cy="${y}" r="2.5" fill="#FF4060"/>`).join('')} <defs><radialGradient id="rg_admin" cx="50%" cy="40%" r="55%"> <stop offset="0%" stop-color="#FF7080"/><stop offset="100%" stop-color="#800020"/> </radialGradient></defs> <circle cx="40" cy="40" r="32" fill="url(#rg_admin)"/> <circle cx="40" cy="40" r="32" fill="none" stroke="#FF4060" stroke-width="2" opacity="0.7"/> <text x="40" y="46" text-anchor="middle" font-size="28" font-family="sans-serif">🛡</text> </svg>`;
}

function founderBadgeSVG() {
const pts = [[40,5],[62,12],[75,34],[75,46],[62,68],[40,75],[18,68],[5,46],[5,34],[18,12]];
return `<svg width="56" height="56" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg"> <circle cx="40" cy="40" r="38" fill="#102060"/> ${pts.map(([x,y])=>`<circle cx="${x}" cy="${y}" r="2.5" fill="#80C0FF"/>`).join('')} <defs><radialGradient id="rg_found" cx="50%" cy="40%" r="55%"> <stop offset="0%" stop-color="#C0E8FF"/><stop offset="100%" stop-color="#2030A0"/> </radialGradient></defs> <circle cx="40" cy="40" r="32" fill="url(#rg_found)"/> <circle cx="40" cy="40" r="32" fill="none" stroke="#A0D8FF" stroke-width="2"/> <polygon points="40,20 43,30 54,30 45,37 48,47 40,41 32,47 35,37 26,30 37,30" fill="#FFD040" opacity="0.9"/> </svg>`;
}

function legendBadgeSVG() {
const pts = [[40,5],[62,12],[75,34],[75,46],[62,68],[40,75],[18,68],[5,46],[5,34],[18,12]];
return `<svg width="56" height="56" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg"> <circle cx="40" cy="40" r="38" fill="#200060"/> ${pts.map(([x,y])=>`<circle cx="${x}" cy="${y}" r="2.5" fill="#C080FF"/>`).join('')} <defs><radialGradient id="rg_legend" cx="50%" cy="40%" r="55%"> <stop offset="0%" stop-color="#E0B8FF"/><stop offset="50%" stop-color="#8040D0"/><stop offset="100%" stop-color="#2800A0"/> </radialGradient></defs> <circle cx="40" cy="40" r="32" fill="url(#rg_legend)"/> <circle cx="40" cy="40" r="32" fill="none" stroke="#D090FF" stroke-width="2.5" opacity="0.8"/> <circle cx="40" cy="40" r="27" fill="none" stroke="#7030B0" stroke-width="1" opacity="0.5"/> <polygon points="40,22 43,31 53,31 45,37 48,46 40,41 32,46 35,37 27,31 37,31" fill="#FFD040" opacity="0.95"/> <circle cx="40" cy="37" r="5" fill="#FFF0A0" opacity="0.6"/> </svg>`;
}

/* ── Level badge SVG (small corner badge) ── */
function levelBadgeSVG(level) {
const tier = Math.floor(level / 20);
const configs = [
...
const wingsL = cf.wings ? `<path d="M5,14 C0,10 -4,12 -3,16 C-2,19 2,18 4,17 C0,20 -2,24 1,25 C4,26 7,21 9,18" fill="#E8A000" opacity="0.95"/>` : ‘’;
const wingsR = cf.wings ? `<path d="M27,14 C32,10 36,12 35,16 C34,19 30,18 28,17 C32,20 34,24 31,25 C28,26 25,21 23,18" fill="#E8A000" opacity="0.95"/>` : ‘’;
const pedestal = cf.base ? `<rect x="10" y="29" width="12" height="4" rx="1" fill="#7818B8" stroke="#FFD700" stroke-width="0.5"/>` : ‘’;
return `<svg viewBox="-6 0 44 36" xmlns="http://www.w3.org/2000/svg"> ${wingsL}${wingsR} <polygon points="${pts}" fill="${cf.fill}" stroke="${cf.stroke}" stroke-width="1.5"/> <polygon points="${pts}" fill="rgba(255,255,255,0.1)"/> <text x="16" y="21" text-anchor="middle" font-size="9" font-weight="700" fill="white" font-family="sans-serif">${level}</text> ${pedestal} </svg>`;
}
...
if (!uid) return false;
try {
return typeof ADMIN_UIDS !== ‘undefined’ &&
Array.isArray(ADMIN_UIDS) &&
ADMIN_UIDS.indexOf(uid) >= 0;
