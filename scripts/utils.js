...
/* ── Admin: شارة واحدة فقط لكل مجموعة (الأعلى) ── */
...
return ‘<circle cx="' + p[0] + '" cy="' + p[1] + '" r="2.5" fill="' + c.dot + '"/>’;
}).join(’’);
}
...
extraRing = extraRing || ‘’;
const c = TIER_COLORS[t];
return ‘<circle cx="40" cy="40" r="38" fill="' + c.outer + '"/>’ +
tierDots(t) +
‘<defs><radialGradient id="rg_' + t + '" cx="50%" cy="40%" r="55%">’ +
...
const extras = tier >= 3
? ‘<circle cx="40" cy="40" r="27" fill="none" stroke="' + c.ring + '" stroke-width="1" opacity="0.4"/>’
: ‘’;
const diamond = tier === 4
? ‘<polygon points="40,62 46,67 40,74 34,67" fill="#A0DFFF" opacity="0.9"/>’
: ‘’;
...
const starCol  = tier===4 ? ‘#D090FF’ : ‘#FFD040’;
const star     = tier >= 3
? ‘<polygon points="40,25 41.5,28.5 45,28.5 42.5,31 43.5,35 40,33 36.5,35 37.5,31 35,28.5 38.5,28.5" fill="' + starCol + '"/>’
: ‘’;
const sparkle  = tier === 4
? ‘<line x1="28" y1="26" x2="28" y2="30" stroke="#D090FF" stroke-width="1.5" opacity="0.7"/>’ +
‘<line x1="26" y1="28" x2="30" y2="28" stroke="#D090FF" stroke-width="1.5" opacity="0.7"/>’
: ‘’;
return ‘<svg width="56" height="56" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg">’ +
tierBase(tier) +
...
const dotStroke  = tier >= 3 ? c.dot : ‘white’;
const starCol    = tier===4 ? ‘#D090FF’ : ‘#FFD040’;
const star       = tier >= 3
? ‘<polygon points="40,19 41.5,22.5 45,22.5 42.5,25 43.5,28.5 40,27 36.5,28.5 37.5,25 35,22.5 38.5,22.5" fill="' + starCol + '"/>’
: ‘’;
return ‘<svg width="56" height="56" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg">’ +
tierBase(tier) +
...
‘<circle cx="57" cy="58" r="10" fill="' + badgeColor + '" stroke="' + dotStroke + '" stroke-width="1.5"/>’ +
...
return ‘<circle cx="' + p[0] + '" cy="' + p[1] + '" r="2.5" fill="#FF4060"/>’;
}).join(’’);
return ‘<svg width="56" height="56" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg">’ +
‘<circle cx="40" cy="40" r="38" fill="#300010"/>’ + dots +
‘<defs><radialGradient id="rg_admin" cx="50%" cy="40%" r="55%">’ +
...
const pts = [[40,5],[62,12],[75,34],[75,46],[62,68],[40,75],[18,68],[5,46],[5,34],[18,12]];
const dots = pts.map(function(p) {
return ‘<circle cx="' + p[0] + '" cy="' + p[1] + '" r="2.5" fill="#80C0FF"/>’;
}).join(’’);
return ‘<svg width="56" height="56" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg">’ +
‘<circle cx="40" cy="40" r="38" fill="#102060"/>’ + dots +
‘<defs><radialGradient id="rg_found" cx="50%" cy="40%" r="55%">’ +
...
const pts = [[40,5],[62,12],[75,34],[75,46],[62,68],[40,75],[18,68],[5,46],[5,34],[18,12]];
const dots = pts.map(function(p) {
return ‘<circle cx="' + p[0] + '" cy="' + p[1] + '" r="2.5" fill="#C080FF"/>’;
}).join(’’);
return ‘<svg width="56" height="56" viewBox="0 0 80 80" xmlns="http://www.w3.org/2000/svg">’ +
‘<circle cx="40" cy="40" r="38" fill="#200060"/>’ + dots +
‘<defs><radialGradient id="rg_legend" cx="50%" cy="40%" r="55%">’ +
‘<stop offset="0%" stop-color="#E0B8FF"/>’ +
‘<stop offset="50%" stop-color="#8040D0"/>’ +
‘<stop offset="100%" stop-color="#2800A0"/>’ +
‘</radialGradient></defs>’ +
